...
`.github/workflows/ci.yml` runs the `integration` job — it spins up Postgres,
applies the `migrations-postgres/` files, and smoke-tests the collector. A
failure there usually means a migration drifted out of parity.
...
4. Open a PR. The CI integration job verifies the migration applies cleanly
