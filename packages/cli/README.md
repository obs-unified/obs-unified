...
- `OBS_COLLECTOR_URL` — default for `keys` and `doctor` (otherwise
  `http://localhost:8790`).
- `OBS_DOCTOR_ORIGINS` — comma-separated origins to test during `doctor` CORS
  checks (defaults to `http://localhost:5173,http://localhost:8080`).
