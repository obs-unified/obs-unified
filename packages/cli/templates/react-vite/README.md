...
Scaffolded by `obs-unified create`. React + Vite frontend + Hono
backend, pre-wired with `@obs-unified/analytics-sdk` and
...
Open `http://localhost:5173`. Click the button. Then open your
obs-unified dashboard (`__COLLECTOR_URL__`) and you'll see:

- A usage event on the **Usage** tab
- A root span + child spans on the **Traces** tab, both stamped
...
- Read [docs/instrumenting](https://obs-unified-docs.dev/docs/instrumenting)
...
- Add `withChildSpan("db.query", fn)` around DB calls — they'll show
  up as child spans under the root.
