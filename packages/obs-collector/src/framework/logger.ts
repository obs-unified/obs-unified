 ...
 *
 * Structurally compatible with `@obs/telemetry-sdk`'s `Logger`, so the worker
 * entrypoint can do:
 *
 ...
 *
 * Structurally compatible with `ChildSpan` from `@obs/telemetry-sdk` so the
 ...
 * Structurally compatible with `withChildSpan` from `@obs/telemetry-sdk`.
