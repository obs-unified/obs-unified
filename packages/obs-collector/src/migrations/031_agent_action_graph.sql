...
CREATE TABLE IF NOT EXISTS actions (
  ...
  total_cost_usd REAL,                       -- Denormalized token/tool cost
  attrs_json TEXT                            -- Arbitrary JSON key-value store for plugin/custom data
);
...
CREATE TABLE IF NOT EXISTS agent_runs (
  ...
  metadata_json TEXT                         -- Opt-in debug metadata
);
...
CREATE TABLE IF NOT EXISTS tool_calls (
  ...
  args_redacted TEXT,                        -- Opt-in redacted parameters JSON
  result_redacted TEXT                       -- Opt-in redacted result JSON
);
...
CREATE TABLE IF NOT EXISTS retrieval_events (
  ...
  documents_json TEXT,                       -- Array of { docId, score, sourceId, snippet_preview }
  total_results INTEGER NOT NULL DEFAULT 0,
  max_relevance_score REAL,
  duration_ms INTEGER
);
...
CREATE TABLE IF NOT EXISTS eval_results (
  ...
  score REAL,                                -- Numeric metric between 0.0 and 1.0
  ...
  rubric_json TEXT                           -- Optional grader schema or rubric details
);
