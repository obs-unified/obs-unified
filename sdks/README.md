...
- OpenInference-shaped helpers for LLM and tool-call spans (so the AI tab in the
...
OpenTelemetry ecosystem of your language. Our SDKs are ergonomics + conventions,
not coverage.
...
## What you do _not_ get from these SDKs

- HTTP server / client instrumentation. Use OTel's
  `@opentelemetry/instrumentation-http` (Node), `otelhttp` (Go), `tower-http` +
...
- Domain-specific span boundaries. You add those manually with the OTel tracer
  at points where blame-attribution matters.
